...
cdef class SLAM:
    ...
    def __cinit__(self, vocab_file, settings_file, sensor, init_pose, use_viewer):
        ...
        self.sys = new System(vocab_file, settings_file, self.sensor, use_viewer, 0, b'', b'', np2Mat(init_pose))

    def track(self, *inputs, timestamp=0.0):
        if self.sensor == RGBD:
            frame, depth_frame = inputs
            self.sys.TrackRGBD(rgb2Mat(frame), np2Mat(depth_frame), timestamp, b'')
